...
fn accept_with_retry_timeout(listen_fd: i32, max_retry: usize) -> Result<i32, Error> {
    ...
    loop {
        match socket::accept(listen_fd) {
            // nix 0.30 returns an OwnedFd; transfer ownership to the caller as a
            // RawFd to preserve 0.8.0's RawFd-based accept contract (caller closes).
            Ok(fd) => return Ok(std::os::unix::io::IntoRawFd::into_raw_fd(fd)),
            Err(e) => {
